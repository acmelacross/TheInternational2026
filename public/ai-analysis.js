  ...
  async function loadAnalysis(){
    ...
    try{
      ...
      if(cr.ok && cached.found){
        ...
        if(note) note.textContent='本场暂无本地分析缓存，将对已配置模型各调用一次并持久缓存。';
        if(grid) grid.innerHTML='<div class="ai-loading"><span></span><b>首次生成多模型分析</b><p>仅首次可能需要几十秒，完成后刷新页面和重新部署都直接读取本地缓存。</p></div>';
      }
      ...
      if(grid && !grid.children.length) grid.innerHTML=`<div class="empty">AI 分析读取失败：${esc(e.message)}</div>`;
